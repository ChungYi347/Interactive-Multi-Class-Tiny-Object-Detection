...
from mmdet.models.losses.uel import UEL
...
@HEADS.register_module
class SharedFCBBoxHeadRbboxUserinput(ConvFCBBoxHeadRbbox):

    def __init__(self, num_fcs=2, fc_out_channels=1024, *args, **kwargs):
        mark_size = kwargs.pop('mark_size')
        loss_userinput = kwargs.pop('loss_userinput')
        super(SharedFCBBoxHeadRbboxUserinput, self).__init__(
            num_shared_convs=0,
            num_shared_fcs=num_fcs,
            num_cls_convs=0,
            num_cls_fcs=0,
        ...
        loss_userinput = build_loss(loss_userinput) if loss_userinput is not None else None
        self.uel = UEL(mark_size, self.num_classes, self.target_means, self.target_stds, loss_userinput, self.reg_class_agnostic, True)
    ...
    def loss(self,
             ...
             img_metas=None,
             gt_userinputs=None):
        losses = super().loss(cls_score, bbox_pred, labels, label_weights,
                                bbox_targets, bbox_weights)
        losses.update(self.uel.loss(img_metas, labels, label_weights, bbox_pred, rois, cls_score, gt_userinputs))
        return losses
